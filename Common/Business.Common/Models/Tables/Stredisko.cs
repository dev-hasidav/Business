...
namespace Business.Models.Tables
{
    [NumClass(70)]
    [Serializable]
    public class Stredisko : Interfases.BaseModelTable
    {
        #region  ==========  Constructor  ==========

        public Stredisko() : base(Actions.SyncStr) { }
        public Stredisko(int Id, Models.Servers Server, string BaseData, string Ico) : base(Id, Server, BaseData, Ico, Actions.SyncStr) { }
        public Stredisko(string Name, Models.Servers Server, string BaseData, string Ico) : base(Name, Server, BaseData, Ico, Actions.SyncStr) { }
        ...
        private List<Stredisko> _CollectionStredisko = new List<Stredisko>();
        /// <summary>
        /// Коллекция для чтения и записи блока данных. Про клмманде 'SET' устанавливает 'IsRecord = NumberRecord.Many'
        /// </summary>
        public List<Stredisko> CollectionStredisko { get { return _CollectionStredisko; } set { _CollectionStredisko = value; IsRecord = NumberRecord.Many; } }
        /// <summary>
        ...
        protected override void LoadSql(int Id, string Ids)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    this.CollectionStredisko.Add(cr);
                    cr.IsRecord = NumberRecord.Many;
                }
                dr.Close();
                if (IsSelect && this.CollectionStredisko.Count != 0)
                {
                    this.Id = this.CollectionStredisko[0].Id;
                    this.Name = this.CollectionStredisko[0].Name;
                    this.Ids = this.CollectionStredisko[0].Ids;
                    this.Remark = this.CollectionStredisko[0].Remark;
        ...
        protected override void CreateSql()
        {
            ...
            try
            {
                ...
                {
                    foreach (var item in this.CollectionStredisko)
                    {
        ...
        /// <returns></returns>
        public static Stredisko GetStredisko(Servers Srv, string NameBase, int Id)
        {
            Stredisko cou = new Stredisko(Id, Srv, NameBase, "");
            return cou;
        }
        ...
        /// <returns></returns>
        public static Stredisko GetStredisko(Servers Srv, string NameBase, string Ids)
        {
            Stredisko cou = new Stredisko(Ids, Srv, NameBase, "");
            return cou;
        }
        ...
        /// <returns></returns>
        public static List<Stredisko> GetList(Servers Srv, string BaseName)
        {
            List<Stredisko> li = new List<Stredisko>();
            Stredisko bn1 = new Stredisko(0, Srv, BaseName, "");
            foreach (var item in bn1.CollectionStredisko)
            {
        ...
        public static Dictionary<string, Stredisko> GetChashStrediskoIds(Servers Srv, string BaseName)
        {
            Dictionary<string, Stredisko> di_id = new Dictionary<string, Stredisko>();
            Stredisko bn = new Stredisko(0, Srv, BaseName, null);
            foreach (Stredisko pb in bn.CollectionStredisko)
            {
        ...
        public static Dictionary<int, Stredisko> GetChashPartnerId(Servers Srv, string BaseName)
        {
            Dictionary<int, Stredisko> di_id = new Dictionary<int, Stredisko>();
            Stredisko bn = new Stredisko(0, Srv, BaseName, null);
            foreach (Stredisko pb in bn.CollectionStredisko)
            {
                di_id.Add(pb.Id, pb);
