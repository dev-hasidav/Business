...
namespace Business.Models.Tables
{
    [NumClass(71)]
    [Serializable]
    public class FormUhrada : Interfases.BaseModelTable
    {
        #region  ==========  Constructor  ==========

        public FormUhrada() : base(Actions.SyncFornUh) { }
        public FormUhrada(int Id, Models.Servers Server, string BaseData, string Ico) : base(Id, Server, BaseData, Ico, Actions.SyncFornUh) { }
        public FormUhrada(string Name, Models.Servers Server, string BaseData, string Ico) : base(Name, Server, BaseData, Ico, Actions.SyncFornUh) { }
        ...
        private List<FormUhrada> _CollectionFormUhrada = new List<FormUhrada>();
        /// <summary>
        /// Коллекция для чтения и записи блока данных. Про клмманде 'SET' устанавливает 'IsRecord = NumberRecord.Many'
        /// </summary>
        public List<FormUhrada> CollectionFormUhrada { get { return _CollectionFormUhrada; } set { _CollectionFormUhrada = value; IsRecord = NumberRecord.Many; } }
        /// <summary>
        ...
        protected override void LoadSql(int Id, string Ids)
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    this.CollectionFormUhrada.Add(cr);
                    cr.IsRecord = NumberRecord.Many;
                }
                dr.Close();
                if (IsSelect && this.CollectionFormUhrada.Count != 0)
                {
                    this.Id = this.CollectionFormUhrada[0].Id;
                    this.Name = this.CollectionFormUhrada[0].Name;
                    this.Ids = this.CollectionFormUhrada[0].Ids;
                    this.Remark = this.CollectionFormUhrada[0].Remark;
        ...
        protected override void CreateSql()
        {
            ...
            try
            {
                ...
                {
                    foreach (var item in this.CollectionFormUhrada)
                    {
        ...
        /// <returns></returns>
        public static FormUhrada GetFormUhrada(Servers Srv, string NameBase, int Id)
        {
            FormUhrada cou = new FormUhrada(Id, Srv, NameBase, "");
            return cou;
        }
        ...
        /// <returns></returns>
        public static FormUhrada GetFormUhrada(Servers Srv, string NameBase, string Ids)
        {
            FormUhrada cou = new FormUhrada(Ids, Srv, NameBase, "");
            return cou;
        }
        ...
        /// <returns></returns>
        public static List<FormUhrada> GetList(Servers Srv, string BaseName)
        {
            List<FormUhrada> li = new List<FormUhrada>();
            FormUhrada bn1 = new FormUhrada(0, Srv, BaseName, "");
            foreach (var item in bn1.CollectionFormUhrada)
            {
        ...
        public static Dictionary<string, FormUhrada> GetChashFormUhradaIds(Servers Srv, string BaseName)
        {
            Dictionary<string, FormUhrada> di_id = new Dictionary<string, FormUhrada>();
            FormUhrada bn = new FormUhrada(0, Srv, BaseName, null);
            foreach (FormUhrada pb in bn.CollectionFormUhrada)
            {
        ...
        public static Dictionary<int, FormUhrada> GetChashPartnerId(Servers Srv, string BaseName)
        {
            Dictionary<int, FormUhrada> di_id = new Dictionary<int, FormUhrada>();
            FormUhrada bn = new FormUhrada(0, Srv, BaseName, null);
            foreach (FormUhrada pb in bn.CollectionFormUhrada)
            {
                di_id.Add(pb.Id, pb);
